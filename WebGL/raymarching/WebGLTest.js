...
(() => {
	...
	var near = 10;
	var far = 100000;
	var projectMatrix = MyMatrix.persperctiveMatrix(fov, cv.width / cv.height, near, far);
	var pivot = new Float32Array([0, 0, 0]);
	const modelTransfrom = new MyMatrix();
	const cameraTransform = new MyMatrix();
	...
	function draw() {
		...
		if (obj !== null) {
			...
			gl.uniformMatrix4fv(material.uniforms.uM, false, modelTransfrom.data);
			gl.uniformMatrix4fv(material.uniforms.uV, false, cameraTransform.inverse().data);
			gl.uniformMatrix4fv(material.uniforms.uP, false, projectMatrix.data);
	...
	function OnCanvasResize(e, forceResize = false) {
		if (cv.height != cv.clientHeight || cv.width != cv.clientWidth || forceResize) {
			...
			projectMatrix = MyMatrix.persperctiveMatrix(fov, cv.width / cv.height, near, far);
		}
	}
	...
	function zoomToFit() {
		let invc = cameraTransform.inverse();
		...
		let offset = Math.max(pp[0], pp[1]) - pp[3];
	...
	document.getElementById("file").addEventListener("change", ev => {
		...
		createVAOFromSTLFile(input.files[0], gl).then(stl => {
			obj = stl;
			centerPivot();
			zoomToFit();
		}
